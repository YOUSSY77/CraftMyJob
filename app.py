...
def select_discriminant_skills(text: str,
    ...
    scored = sorted({(t, idf_map.get(t, 0)) for t in tokens},
...
choices = st.multiselect("Générations IA", list(tpls.keys()),
                         default=list(tpls.keys())[:2])
...
if st.button("🚀 Lancer tout"):
    ...
    if cv_text:
        summary = get_gpt_response(
            f"Résumé en 5 points clés du CV suivant:\n{cv_text[:1000]}",
            key_openai
        )
