# -*- coding: utf-8 -*-
import os
# ── 0) Supprimer tout proxy hérités
...
# ── Chargement des communes pour autocomplete
@st.cache_data
def load_communes():
    df = pd.read_csv("datacommunes.csv", dtype=str)
    # Ajustez ces noms de colonnes selon votre CSV si nécessaire
    df["ville_cp"] = df["Nom_de_la_commune"].str.strip() + " (" + df["Code_postal"].str.strip() + ")"
    return df

communes_df = load_communes()

# ── Chargement du référentiel métiers
@st.cache_data
def load_metiers():
...
suggestions = communes_df[
    communes_df["Nom_de_la_commune"].str.contains(typed, case=False, na=False)
]["ville_cp"].tolist()[:10]
locations = st.multiselect("Sélectionnez une ou plusieurs villes", options=suggestions)

# Extraire les codes postaux pour Pôle-Emploi
postal_codes = []
...
choices = st.multiselect("Choisis les éléments à générer", list(templates.keys()), default=list(templates.keys())[:2])
...
if st.button("🚀 Lancer tout"):
    ...
    # — Offres pour le poste saisi
    token      = fetch_ft_token(ft_client_id, ft_secret)
    mots_cles  = f"{job_title} {skills}"
    st.subheader(f"🔎 Offres pour « {job_title} »")
    all_offres = []
    for pc in postal_codes:
        all_offres += search_offres(token, mots_cles, pc, limit=5)
    seen, uniq = set(), []
    for o in all_offres:
    ...
    # — SIS : matching métiers + leurs offres
    ...
    for _, r in top6.iterrows():
        ...
        # pour chaque métier, top 3 offres
