...
<style>
  ...
  .cv-summary { background-color:#e8f5e9; padding:10px; border-radius:4px; }
</style>
...
def search_territoires(query: str, limit=10):
    ...
    if re.fullmatch(r"\d{2}", query):
        # département → toutes ses communes
...
def search_offres(token: str, mots: str, lieu: str, limit=5) -> list:
    ...
    params = {"motsCles":mots,"localisation":lieu,"range":f"0-{limit-1}",
              "dateDebut":dateDebut,"dateFin":dateFin,"tri":"dateCreation"}
...
choices = st.multiselect("Que générer ?", list(tpls.keys()), default=list(tpls.keys())[:2])
...
if st.button("🚀 Lancer tout"):
    ...
    if not (key_pe_id and key_pe_secret and sel):
        st.error("🔑 Identifiants Pôle-Emploi + territoires requis"); st.stop()

    # 1) Résumé automatique du CV
    cv_summary = ""
    if cv_text:
        cv_summary = get_gpt_response(
            "Résume ce CV en 5 points clés, style bullet-points.",
            key_openai
        )
        st.markdown("<div class='cv-summary'><strong>📝 Résumé CV :</strong><br>" +
                    cv_summary.replace("\n", "<br>") +
    ...
    for name in choices:
        ...
        if name == "📄 Bio LinkedIn":
            instr += " Ne mentionne aucune localisation (pas de 'basé à ...') et fais une bio de maximum 4 lignes."
        prompt_lines = []
        if cv_summary:
            prompt_lines.append(f"Résumé CV : {cv_summary}")
        prompt_lines += [
            f"Poste: {job_title}",
            f"Missions: {missions}",
        ...
        try:
            ...
            if name == "🧩 CV optimisé IA":
                ...
                st.download_button("📥 Télécharger CV optimisé",
                                   data=pdf, file_name="CV_optimisé.pdf",
    ...
    for o in all_offres:
        ...
        if url not in uniq:
            uniq[url] = o
    # Fuzzy match titre
    matches = [o for o in uniq.values()
               if fuzz.token_set_ratio(o.get("intitule",""), job_title) >= 60]
    display = matches or list(uniq.values())
    if display:
        for o in display[:5]:
            ...
            st.markdown(
                f"**{title}** ({typ}) – {lib} [{cp}]  \n"
                f"<span class='offer-link'><a href='{link}' target='_blank'>Voir l'offre</a></span>\n---",
                unsafe_allow_html=True
            )
    else:
    ...
    for _, r in top6.iterrows():
        ...
        kws = r["Metet"] if False else r["Metier"]  # ensure correct key
        subs = []
        for loc in sel:
            loc_norm = normalize_location(loc)
        ...
        if subs:
            for o in subs:
                link2 = o.get("contact",{}).get("urlPostulation") or o.get("contact",{}).get("urlOrigine","#")
                if link2 in seen2: continue
                seen2.add(link2)
                ...
                st.markdown(
                    ...
                    f"{desc}  \n"
                    f"<span class='offer-link'><a href='{link2}' target='_blank'>Voir / Postuler</a></span>",
                    unsafe_allow_html=True
                )
        else:
            st.info("Aucune offre pour ce métier dans vos territoires et contrats.")
