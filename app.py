...
def normalize_location(loc: str) -> str:
    ...
    if m:
        return m.group(1)
    m2 = re.match(r"Département (\d{2})", loc)
    if m2:
        return m2.group(1)
...
# compute date range for last 2 months
...
def build_keywords(texts: list[str], max_terms: int = 7) -> str:
    ...
    stop = set(["et","ou","la","le","les","de","des","du","un","une","à","en","pour","par","avec","sans","sur","dans","au","aux"])
    seen, kws = set(), []
    for t in tokens:
...
# modified search_offres to include date window
...
eexp=st.radio("🎯 Expérience", ["Débutant (0-2 ans)","Expérimenté (2-5 ans)","Senior (5+ ans)"])
...
tpls={"📄 Bio LinkedIn":"Rédige une bio LinkedIn professionnelle.","✉️ Mail candidature":"Écris un mail de candidature spontanée.","📃 Mini CV":"Génère un mini-CV (5-7 lignes).","🧩 CV optimisé IA":"Optimise le CV en soulignant deux mots-clés."}
...
if st.button("🚀 Lancer tout"):
    ...
    if not (key_pe_id and key_pe_secret and sel): st.error("Pôle-Emploi & territoires requis"); st.stop()
    ...
    for name in choices:
        prompt = "\n".join([
            ...
            f"Territoires: {', '.join(sel)}", f"Expérience: {eexp}", f"Contrat: {contract}", f"Télétravail: {'Oui' if remote else 'Non'}", "", tpls[name]
        ])
        try:
    ...
    if unique:
        for url,o in list(unique.items())[:5]:
            lib=o['lieuTravail']['libelle']; title=o['intitule']
    ...
    for _,r in top6.iterrows():
        ...
        if subs:
            for o in subs:
                ...
                if link not in seen:
                    seen.add(link)
                    dt=o.get('dateCreation','')[:10]; lib=o['lieuTravail']['libelle']
                    desc=(o.get('description','') or '').replace("\n"," ")[:150]+"…"
                    st.markdown(f"• **{o['intitule']}** – {lib} (_Publié {dt}_)  \n{desc}  \n<span class='offer-link'><a href='{link}' target='_blank'>Voir</a></span>", unsafe_allow_html=True)
        else:
